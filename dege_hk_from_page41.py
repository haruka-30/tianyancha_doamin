...
def mak_console_log():
    ...
    # 使用示例
    sys.stdout = DualOutput("consoleLog/log_hk_from_page41.txt")
    sys.stderr = DualOutput("consoleLog/log_hk_from_page41.txt")
...
def merge_domain():
    # 获取到当前时间，构建文件名
    txtname = f"hk_domains_from_page{START_PAGE}_" + mak_date_folder() + ".txt"
    # 设定源文件夹和目标文件夹路径
    source_folder = "tmp"
    ...
    print(f"总共获取 .hk 域名数: {len(unique_lines)}")


def get_end_nub(driver):
    time.sleep(2)  # 先等待2秒，确保页面加载完成
...
def main():
    ...
    delete_tmp()  # 删除tmp目录
    page = START_PAGE  # 从第41页开始
    search = SEARCH_KEYWORD  # 固定搜索 .hk
    driver.maximize_window()  # 最大化窗口
    login_web(driver)  # 打开页面并手动登录，传递driver

    result = check_man_machine_verification(driver)  # 检查人机验证
    if not result:
        print(f"验证通过，开始从第 {START_PAGE} 页爬取 '{search}' 域名...")
        ...
        if len(elements) > 0:
            ...
            # 默认标签数
            label = 0
            print(f"总页数: {superend}, 开始页: {START_PAGE}")
            if superend < Maximum_tag:
                # 在注册窗口之前创建一个进度条实例
                pbar = tqdm(
                    total=superend - START_PAGE + 1,
                    initial=0,
                    desc="爬取进度：",
                    unit="页",
                )
                # 从START_PAGE开始爬取到最后一页
                for page in range(START_PAGE, superend + 1):
                    label += 1
                    time.sleep(2)  # 等待2秒
            ...
            if verify_elements:
                print("登录时出现人机验证，请20秒内完成验���，然后程序会自动重启")
                time.sleep(20)
                # 重启当前程序
                os.startfile(__file__)
                sys.exit()
